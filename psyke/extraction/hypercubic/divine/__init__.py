...
class DiViNE(HyperCubeExtractor):
    ...
    def __init__(self, predictor, k: int = 5, patience: int = 15, discretization=None, normalization=None):
    ...
    def _extract(self, dataframe: pd.DataFrame, mapping: dict[str: int] = None, sort: bool = True) -> Theory:
        ...
        while len(data) > 0:
            ...
            while patience > 0 and len(data) > 0:
                tree = BallTree(data.iloc[:, :-1], leaf_size=2)
                _, idx = tree.query([list(cube.center().dimensions.values())], k=1)
                other, data = DiViNE.__pop(data, idx[0][-1])
