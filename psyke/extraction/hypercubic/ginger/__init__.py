...
class GInGER(HyperCubeExtractor):
    ...
    def __init__(self, predictor, features, min_max_slices=(0, 1), min_max_rules=(1, 999), max_poly=1, alpha=0.5,
                 indpb=0.5, tournsize=3, metric='R2', n_gen=50, n_pop=50, threshold=None, valid=None,
    ...
    def _extract(self, dataframe: pd.DataFrame) -> Theory:
        ...
        for poly in range(self.poly):
            for slices in list(itertools.product(range(self.min_max_slices[0], self.min_max_slices[1] + 1),
                ...
                gr = GIn((dataframe.iloc[:, :-1], dataframe.iloc[:, -1]), self.valid, self.features[mask],
                ...
                best[(score, poly + 1, sum(slices), slices)] = b
        m = max(best)
        poly, slices, best = m[1], m[3], best[m]
        ...
        cuts = [sorted(best[sum(slices[:i]):sum(slices[:i + 1])]) for i in range(len(slices)) if mask[i]]

        features = self.features[mask]

        intervals = [[(transformed[features[i]].min() - HyperCube.EPSILON * 2, cut[0])] +
                     [(cut[i], cut[i + 1]) for i in range(len(cut) - 1)] +
                     [(cut[-1], transformed[features[i]].max() + HyperCube.EPSILON * 2)] for i, cut in enumerate(cuts)]

        hypercubes = [{f: iv for f, iv in zip(features, combo)} for combo in itertools.product(*intervals)]
        mi_ma = {f: (transformed[f].min(), transformed[f].max()) for f in transformed.columns if f not in features}
        for f in self._surrounding.dimensions:
            mi_ma[f] = self._surrounding[f]
        self._hypercubes = [self._default_cube({feat: h[feat] if feat in features else mi_ma[feat]
        ...
        for c in self._hypercubes:
            for feature in transformed.columns:
                if feature not in features:
                    ...
                    if c[feature][0] - HyperCube.EPSILON * 3 <= self._surrounding[feature][0]:
                        c.set_infinite(feature, '-')
                    if c[feature][1] + HyperCube.EPSILON * 3 >= self._surrounding[feature][1]:
                        c.set_infinite(feature, '+')
