...
from psyke.gui import TaskBox, DataBox, DataInfoBox, PredictorParameterBox, PredictorInfoBox, PredictorBox, \
    ...
    TitleBox, VerticalBoxLayout, HorizontalBoxLayout, DataManipulationBox
    ...
    def train_extractor(self, widget):
        ...
        if extractor == 'GridEx':
            ...
            self.extractor = Extractor.cart(
                predictor=CartPredictor(self.model), simplify=params.get('simplify', True))
        elif extractor == 'CReEPy':
            output = ClusterExtractor.Target.CLASSIFICATION if self.task == 'C' else \
                ClusterExtractor.Target.CONSTANT if params.get('output', False) else ClusterExtractor.Target.REGRESSION
